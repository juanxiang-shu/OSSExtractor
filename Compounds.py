...
def find_words(paragraph):
    ...
    # matches1 = re.findall(temperature_pattern1, paragraph)
    # if matches1:
    #     temperature1 = "300 K"
    element_pattern = r"\b(?:{})\b".format("|".join(element_definitions))
    compound_pattern = r"\b\s*(\S*(?:{})\S*)\s*\b".format("|".join(compounds_words))#r"\b\s*(\S*(?:{})\S*)\s*\b".format("|".join(compounds_words))
    ...
    result = {
        "reaction": reaction,
        "temperature": temperature, #.union(set([temperature1])) if matches1 else temperature
        "substrate": substrate,
